...
class Admin::OverviewController < Admin::BaseController
  ...
  def best_selling_variants
    li = LineItem.sum(:quantity, :group => :variant_id, :limit => row_count)
  ...
  def top_grossing_variants
    quantity = LineItem.sum(:quantity, :group => :variant_id, :limit => row_count)
    prices = LineItem.sum(:price, :group => :variant_id, :limit => row_count)
  ...
  def last_five_orders
    orders = Order.includes(:line_items).where("completed_at IS NOT NULL").order("completed_at DESC").limit(row_count)
    orders.map do |o|
  ...
  def biggest_spenders
    spenders = Order.sum(:total, :group => :user_id, :limit => row_count, :order => "sum(total) desc", :conditions => "completed_at is not null and user_id is not null")
